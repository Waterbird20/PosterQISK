...
\logoleft{\includegraphics[height=4.0cm]{logos/KIST_CI.eps}}
\logoright{\includegraphics[height=2.5cm]{logos/PaulLee2.eps}}
...
\begin{column}{\colwidth}
  ...
  $$
  where $U_0$ and $U_1$ are single-qubit rotations conditioned on the control qubit state. Without loss of generality, we can write each operation as $e^{-i \theta_{i} \vec{\sigma}_{i}}$.
  ...
  The Hamiltonian is
  $$
  H = \omega_L I_z + A_\parallel S_z I_z + A_\perp S_z I_x,
  $$
  ...
  \begin{block}{Dynamic Decoupling Based Conditional Gate}
    ...
    Let $U_0$ and $U_1$ be the conditional evolution operators for a single decoupling unit:
    \begin{align}
      U_0 &= e^{-i\tau H_0}e^{-i2\tau H_1}e^{-i\tau H_0} = e^{-i\phi \vec{\sigma}_0} \\
      U_1 &= e^{-i\tau H_1}e^{-i2\tau H_0}e^{-i\tau H_1} = e^{-i\phi \vec{\sigma}_1}
    \end{align}
    Here $\omega_0 \equiv \omega_L$ and $\omega_1 = \sqrt{(\omega_L + A_\parallel)^2 + A_\perp^2}$ are the nuclear precession frequencies for $m_s = 0$ and $m_s = -1$, respectively.
    ...
    \end{align}
    We can examine the inner product between the two rotation axes. 
    \begin{align}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{DDrf Gate}
    ...
    \begin{align}
      H = \omega_0 I_z + A_{\parallel}S_z I_z + A_{\perp}S_z I_x + 2\Omega_{\text{RF}}\cos(\omega t + \phi) I_x
    \end{align}
    ...
    In the rotating frame at the rf frequency, and under the approximation $A_\perp \approx 0$ and $\Omega_{\text{RF}} \ll (\omega_0 - \omega_1)$ for simplicity:
    \begin{align}
  ...
  \begin{block}{Hybrid DDrf Gate}
    ...
    The speed of the conditional operation is then boosted by additional rf driving on top of the dynamic decoupling contribution.
    Let $\beta = \arcsin(A_\perp / \omega_1)$ be the tilt angle of the nuclear precession axis from the $z$-axis when the electron is in $\ket{1}$.
    In the regime of small $A_\perp$ where $\sin\beta\simeq\beta$, the two conditional rotation angles combine additively, giving $\phi \simeq \phi_{DD} + \phi_{DDrf}$.
    With the initial phase $\varphi = 0$, the resulting conditional gate is
    \begin{align}
      U = \ketbra{0} \otimes U_0 + \ketbra{1} \otimes U_1,
    \end{align}
    where
    ...
    \end{align}
    The two terms in the argument correspond to the DD contribution ($\propto A_\perp$) and the rf contribution ($\propto \Omega_{\text{rf}}$), and can be tuned independently. In particular, for spins with negligible $A_\perp$ where the DD gate alone is ineffective, the rf drive provides the dominant conditional rotation, while for spins with sizable $A_\perp$ the two mechanisms reinforce each other to reduce the total gate time.
  \end{block}
